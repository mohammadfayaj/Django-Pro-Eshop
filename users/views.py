...
# from users.models import DeliveryCharge
...
@login_required
def address_info(request, id):
    obj = Address.objects.get(users=request.user)
    address_info_list = Address.objects.all()
    if request.method == "POST":
        address_info_form = AddressForm(request.POST, instance=obj)
        if address_info_form.is_valid():
            address_info_form.save()
            return redirect('users:users-profile')
    else:
        address_info_form = AddressForm(instance=obj)
    context = {
